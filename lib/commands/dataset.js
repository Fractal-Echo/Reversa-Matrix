...
import { buildLocalToolInventory } from '../../scripts/build-local-tool-inventory.js';
...
export default async function dataset(args) {
  ...
  if (subcommand === 'local-coder-sft') {
    const options = parseLocalCoderSftArgs(rest);
    if (options.help) {
      printLocalCoderSftHelp();
      return;
    }

    for (const [index, path] of options.packs.entries()) {
      if (!existsSync(path)) {
        throw new Error(`Agentic pack ${index + 1} does not exist: ${path}`);
      }
    }
    if (options.corpus && !existsSync(options.corpus)) {
      throw new Error(`Private corpus path does not exist: ${options.corpus}`);
    }

    const result = await buildLocalCoderSftDataset(options);
    console.log(chalk.bold('\n  Reversa local coder SFT dataset complete\n'));
    console.log(`  Output:        ${chalk.cyan(result.outDir)}`);
    console.log(`  Examples:      ${chalk.cyan(result.totalExamples)}`);
    console.log(`  Train/val/test:${chalk.cyan(` ${result.splits.train}/${result.splits.val}/${result.splits.test}`)}`);
    console.log(`  Hashes:        ${chalk.cyan(result.hashesPath)}`);
    console.log('');
    return;
  ...
  if (subcommand === 'local-tool-inventory') {
    const options = parseLocalToolInventoryArgs(rest);
    if (options.help) {
      printLocalToolInventoryHelp();
      return;
    }

    if (!existsSync(options.manifest)) {
      throw new Error(`Local tool manifest does not exist: ${options.manifest}`);
    }

    const result = await buildLocalToolInventory(options);
    console.log(chalk.bold('\n  Reversa local tool inventory complete\n'));
    console.log(`  Output:  ${chalk.cyan(result.outDir)}`);
    console.log(`  Tools:   ${chalk.cyan(result.totalRecords)}`);
    console.log(`  Present: ${chalk.cyan(result.present)}`);
    console.log(`  Missing: ${chalk.cyan(result.missing)}`);
    console.log(`  Hashes:  ${chalk.cyan(result.hashes)}`);
    console.log('');
    return;
  }
...
function parseLocalCoderSftArgs(args) {
  const options = {
    packs: [],
    corpus: null,
    out: null,
    maxCorpusRecords: null,
    maxChars: null,
  ...
  for (let index = 0; index < args.length; index += 1) {
    ...
    switch (flag) {
      ...
      case '--include-local-experimental':
        ...
        break;
      default:
        throw new Error(`Unknown local-coder-sft dataset option: ${arg}`);
    }
  }

  if (!options.help && (!options.out || (!options.packs.length && !options.corpus))) {
    throw new Error('Missing required --out and at least one --pack or --corpus');
  }
  return options;
}

function parseLocalToolInventoryArgs(args) {
  const options = {
    manifest: null,
    out: null,
    maxDepth: null,
  ...
  for (let index = 0; index < args.length; index += 1) {
    ...
    switch (flag) {
      ...
      case '--max-hash-bytes':
        ...
        break;
      default:
        throw new Error(`Unknown local-tool-inventory dataset option: ${arg}`);
    }
  }
...
function printHelp() {
  ...
  Usage:
    ...
    node ./bin/reversa.js dataset local-coder-sft --help
    node ./bin/reversa.js dataset local-tool-inventory --help
`);
}
...
function printPrivateCorpusSearchHelp() {
  ...
  The search ranks existing corpus chunks, derives 24-TET facets from
  deterministic metadata, and does not mutate source files, launch runtimes,
  or call a model endpoint.
`);
}

function printLocalCoderSftHelp() {
  console.log(`
  Build local-only chat SFT data for the Reversa coder LoRA lane.

  Usage:
    node ./bin/reversa.js dataset local-coder-sft \\
  ...
  Options:
    ...
    --max-corpus-records <count>    Limit private corpus rows.
    --max-chars <chars>             Limit corpus excerpt chars.
    --include-local-experimental    Include personal local-only corpus rows.

  Outputs are advisory training data. They remain local-only, are not source
  authority, and should stay under ignored local/private storage.
`);
}

function printLocalToolInventoryHelp() {
  console.log(`
  Build a local-only tool inventory metadata pack.
  ...
  Output is local metadata only: presence, bounded file roles, executable/DLL
  names, representative hashes, and SFT rows that teach boundary handling. The
  command never launches local tools and does not imply redistribution rights.
`);
}
