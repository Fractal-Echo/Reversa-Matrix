...
export class Writer {
  ...
  // Instala arquivos auxiliares gerenciados pelo Reversa em .reversa/.
  // Nunca sobrescreve arquivos fora do manifest durante update.
  async installSupportFiles(engine, {
    force = false,
    modifiedSet = null,
    manifest = null,
    seen = null,
  } = {}) {
    for (const supportFile of engine.supportFiles ?? []) {
      const relTarget = supportFile.target.replace(/\\/g, '/');
      if (seen?.has(relTarget)) continue;
      seen?.add(relTarget);

      const srcPath = join(TEMPLATES_DIR, 'engines', supportFile.template);
      const destPath = join(this.projectRoot, supportFile.target);
      if (!existsSync(srcPath)) continue;

      if (modifiedSet?.has(relTarget)) continue;
      if (force && manifest && existsSync(destPath) && !Object.hasOwn(manifest, relTarget)) {
        continue;
      }

      const content = readFileSync(srcPath, 'utf8');
      if (!force) {
        this._writeNew(destPath, content);
        continue;
      }
