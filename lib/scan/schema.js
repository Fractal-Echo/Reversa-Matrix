...
export function validateEvidenceItem(item, path = 'evidence') {
  ...
  if ('facets' in item) {
    errors.push(...validateEvidenceFacets(item.facets, `${path}.facets`));
  }
  return errors;
}

export function validateEvidenceFacets(facets, path = 'facets') {
  const errors = [];
  requireFields(facets, FACET_REQUIRED_FIELDS, path, errors);
  if (!facets || typeof facets !== 'object') {
    return errors;
  }
  for (const key of FACET_REQUIRED_FIELDS.filter(field => field !== 'deterministic_truth_above_model')) {
    if (typeof facets[key] !== 'string' || facets[key].trim() === '') {
      errors.push(`${path}.${key} must be a non-empty string`);
    }
  }
  if (typeof facets.deterministic_truth_above_model !== 'boolean') {
    errors.push(`${path}.deterministic_truth_above_model must be a boolean`);
  }
  return errors;
}
