...
async function walkProject(dir, context, options, relDir = '') {
  ...
  for (const entry of entries) {
    ...
    if (shouldSkipFile(relPath, rootEntryNames)) {
      context.inventory.counts.files_skipped += 1;
      context.inventory.skipped_files.push({ path: relPath, reason: 'generated_scan_artifact' });
      continue;
    }
...
function shouldSkipFile(relPath, rootEntryNames) {
  if (relPath.includes('/')) {
    return false;
  }
