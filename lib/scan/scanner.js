...
import { lstat, open, readFile, readdir, stat } from 'fs/promises';
import { basename, dirname, extname, join, relative, resolve, sep } from 'path';
import {
  CLAUDE_CODE_BASE_ALIASES,
  CLAUDE_CODE_BASE_PROFILE_ID,
} from '../core/claude-code-base.js';
import { deriveEvidenceFacets } from './facets.js';
...
async function walkProject(dir, context, options, relDir = '') {
  ...
  for (const entry of entries) {
    ...
    await scanPandemoniumInventorySignals(record, context, absPath);
    await scanWindowsPackageInventorySignals(record, context, absPath);
...
async function scanFile(file, context) {
  ...
  scanPandemoniumCnfManifest(file, text, context);
...
function scanLine(line, lineNo, file, context, lineState = {}) {
  ...
  scanDosRuntimeKeywordFamilies(line, lineNo, file, context);
  ...
  scanPandemoniumPcRuntimeLine(line, lineNo, file, context);
  ...
  scanWindowsPackageRuntimeKeywordFamilies(line, lineNo, file, context);
...
function isPlaceholderExample(line, marker, filePath = '') {
  ...
  if (normalizedMarker === 'DUMMY' && /\bDUMMY\.(?:STR|AVI|WAV|BIN)\b/i.test(trimmed)) {
    return true;
  }
...
function scanFileLevelSignals(file, context) {
  ...
  const checks = [
    ...
    {
      pattern: /(^|\/).*(voice|transcri(?:be|ption)|speech|audio[-_ ]?(?:in|out|capture|queue)|whisper|vosk|stt|tts).*$/i,
      category: 'voice_transcription_surface',
      severity: 'MEDIUM',
      claim: `voice_transcription_file:${file.path}`,
      action: 'Keep voice/transcription modules optional, local-only, consent-gated, and free of committed raw audio.',
      rationale: 'Voice and transcript files can contain sensitive content; the lane needs explicit retention and redaction boundaries.',
    },
    {
...
async function scanWindowsPackageInventorySignals(file, context, absPath) {
  if (!isWindowsPackageRuntimeProfile(context.profile)) {
    return;
  }

  const normalizedPath = normalizePath(file.path);
  const name = basename(normalizedPath);
  const sizeText = Number.isFinite(file.size) ? ` size=${file.size}` : '';
  const peMetadata = /\.(?:exe|dll|ocx|sys)$/i.test(name)
    ? await readPeHeaderMetadata(absPath)
    : null;
  const peText = peMetadata
    ? ` pe=${peMetadata.kind}/${peMetadata.machine}`
    : '';
  const extractedText = `${normalizedPath}${sizeText}${peText}`;

  const add = (category, severity, claim, action, rationale, symbols = []) => addEvidence(context, {
    category,
    severity,
    confidence: peMetadata ? 'confirmed' : 'likely',
    source_file: file.path,
    source_line_start: 1,
    source_line_end: 1,
    extracted_text: extractedText,
  ...
  if (/\.exe$/i.test(name)) {
    add(
      'windows_package_binary',
      'HIGH',
      `windows.package.exe:${normalizedPath}${sizeText}${peText}`,
      'Hash the executable and verify architecture, launcher role, and target runtime before swapping package files.',
      'Executable identity controls process bitness, config lookup, and compatible DLL chain.',
      [name, 'exe']
    );
    if (peMetadata) {
      add(
        'windows_package_bitness',
        'HIGH',
        `windows.package.pe:${normalizedPath}:${peMetadata.machine}:${peMetadata.kind}`,
        'Keep all proxy DLLs, helper DLLs, and launch arguments aligned with this process architecture.',
        'A 32-bit process cannot load 64-bit DLLs, and a 64-bit process cannot load 32-bit DLLs.',
        [peMetadata.machine, peMetadata.kind]
      );
    }
  }

  if (/\.(?:dll|ocx|sys)$/i.test(name)) {
    add(
      'windows_package_dll_load_order',
      isProxyOrWrapperDllName(name) ? 'HIGH' : 'MEDIUM',
      `windows.package.dll:${normalizedPath}${sizeText}${peText}`,
      'Classify DLL role, bitness, and load order before adding, replacing, or stacking wrappers.',
      'Old game fixes often fail because support DLLs, proxy DLLs, and wrapper DLLs are mixed without a load-order map.',
      [name, 'dll']
    );
    if (isProxyOrWrapperDllName(name)) {
      add(
        'windows_package_dll_load_order',
        'HIGH',
        `windows.package.proxy_dll:${normalizedPath}`,
        'Treat this as a load-order decision point and document the rollback path before testing.',
        'Proxy and wrapper DLL names are loaded by convention; accidental stacking can hide the real failure layer.',
        [name, 'proxy_dll']
      );
    }
  }

  if (/(^|\/)(?:dosbox|dosbox[-_ ]?x|dosbox[-_ ]?staging).*\.conf$/i.test(normalizedPath)
    || /\.(?:ini|cfg|conf)$/i.test(name)) {
    add(
      'windows_package_runtime_config',
      'MEDIUM',
      `windows.package.config:${normalizedPath}${sizeText}`,
      'Tie this config to the launch executable, package root, and rollback manifest before testing.',
      'Config files are often package-relative and can silently point an otherwise correct runtime at stale paths.',
      [name, 'config']
    );
  }

  if (/\.(?:fx|glsl|hlsl|slang)$/i.test(name)) {
    add(
      'windows_package_shader_asset',
      'MEDIUM',
      `windows.package.shader:${normalizedPath}${sizeText}`,
      'Keep shader fixes in the presentation lane and verify renderer compatibility before enabling them.',
      'Shader assets such as fixvideo-style scanline fixes depend on the selected renderer output path.',
      [name, 'shader']
    );
  }

  if (/\.(?:zip|7z|rar|msi|msix|cab)$/i.test(name)) {
    add(
      'windows_package_layout',
      'HIGH',
      `windows.package.archive:${normalizedPath}${sizeText}`,
      'Inspect archive layout, contained executable names, dependencies, and config defaults before extraction.',
      'Archive layout controls where relative config, DLL, shader, and disc-image paths resolve.',
      [name, 'archive']
    );
  }
}

async function readPeHeaderMetadata(absPath) {
  let handle;
  try {
    handle = await open(absPath, 'r');
    const buffer = Buffer.alloc(4096);
    const { bytesRead } = await handle.read(buffer, 0, buffer.length, 0);
    if (bytesRead < 0x40 || buffer.toString('ascii', 0, 2) !== 'MZ') {
      return null;
    }
    const peOffset = buffer.readUInt32LE(0x3c);
    if (peOffset + 26 > bytesRead || buffer.toString('ascii', peOffset, peOffset + 4) !== 'PE\u0000\u0000') {
      return null;
    }
    ...
    return {
      ...
      machineHex: `0x${machineValue.toString(16).padStart(4, '0')}`,
      kind: optionalMagic === 0x20b ? 'PE32+' : optionalMagic === 0x10b ? 'PE32' : `optional-0x${optionalMagic.toString(16)}`,
    };
  } catch {
    return null;
  } finally {
...
function peMachineName(machineValue) {
  if (machineValue === 0x014c) return 'x86';
  if (machineValue === 0x8664) return 'x64';
  if (machineValue === 0xaa64) return 'arm64';
  if (machineValue === 0x01c4) return 'arm';
  return `machine-0x${machineValue.toString(16)}`;
}

function isProxyOrWrapperDllName(name) {
  return /^(?:dinput8|dxgi|d3d8|d3d9|d3d10|d3d11|d3d12|ddraw|winmm|version|glide|glide2x|vulkan-1|nvngx_dlssg|sl\.dlss_g|sl\.interposer|ReShade64?|SpecialK64?)\.dll$/i.test(name);
}

async function scanPandemoniumInventorySignals(file, context, absPath) {
  if (!isPandemoniumProfile(context.profile)) {
    return;
  }
  ...
  const name = basename(normalizedPath);
  const sizeText = Number.isFinite(file.size) ? ` size=${file.size}` : '';
  const headerProbe = shouldProbePandemoniumPrefix(normalizedPath)
    ? await readFilePrefixProbe(absPath, 64)
    : null;
  const headerHex = headerProbe?.hex ?? null;
  const headerText = headerHex ? ` header32=${headerHex}` : '';
  const extractedText = `${normalizedPath}${sizeText}${headerText}`;
  const add = (category, severity, claim, action, rationale, symbols = []) => addEvidence(context, {
    category,
    severity,
    confidence: 'confirmed',
    ...
    source_line_end: 1,
    extracted_text: extractedText,
    normalized_claim: `${claim}${headerText}`,
    related_paths: [file.path],
    related_symbols: uniq([name, ...symbols].filter(Boolean)),
    evidence_type: 'file_inventory',
    suggested_action: action,
    rationale,
  });

  if (/^(?:PANDY3?|CHARLES)\.EXE$/i.test(name)) {
    add(
      'pandemonium_identity_surface',
      'HIGH',
      `pandemonium.inventory.exe:${normalizedPath}${sizeText}`,
      'Hash and identify this executable before wrapper tests or byte-level research.',
      'Pandemonium executable identity controls the valid wrapper path, registry behavior, and any future patch guards.',
      ['EXE']
    );
  }

  if (/^(?:glide|3DfxSpl|SST1INIT|dgVoodoo|D3D8|D3D9|D3DImm|DDraw|dxgi|d3d11)\.dll$/i.test(name)) {
    add(
      'pandemonium_wrapper_chain',
      'HIGH',
      `pandemonium.inventory.wrapper:${normalizedPath}${sizeText}`,
      'Record DLL bitness and load order before replacing or stacking graphics wrappers.',
      'Wrapper DLL names identify the active Glide/DirectX translation lane, but the load order and x86/x64 boundary still need proof.',
      ['wrapper']
    );
  }

  if (/^XANLIB\.DLL$/i.test(name) || /\.(?:AVI)$/i.test(name)) {
    add(
      'pandemonium_xan_video_assets',
      'MEDIUM',
      `pandemonium.inventory.video:${normalizedPath}${sizeText}`,
      'Treat video playback and replacement as a separate FMV lane with codec, timing, and fallback proof.',
      'Pandemonium uses old AVI/Xan playback assets; video upgrades should not be mixed into render-wrapper fixes.',
      ['FMV']
    );
  }
  ...
  if (/\.PKG$/i.test(name)) {
    add(
      'pandemonium_pkg_asset_archives',
      'HIGH',
      `pandemonium.inventory.pkg:${normalizedPath}${sizeText}`,
      'Map PKG header words and table entries before attempting texture, model, animation, or sound replacement.',
      'PKG archives are the core asset boundary for Pandemonium remaster work; file presence alone is not extraction proof.',
      ['PKG']
    );

    if (headerProbe?.u32Le?.length) {
      addEvidence(context, {
        category: 'pandemonium_pkg_header_probe',
        severity: 'HIGH',
        confidence: 'confirmed',
        source_file: file.path,
        source_line_start: 1,
        source_line_end: 1,
        extracted_text: `${extractedText} u32le=${headerProbe.u32Le.join(',')}`,
        normalized_claim: `pandemonium.pkg.header:${normalizedPath}:u32le=${headerProbe.u32Le.join(',')}`,
        related_paths: [file.path],
        related_symbols: [name, 'PKG_HEADER'],
        evidence_type: 'file_inventory',
        suggested_action: 'Compare PKG header words across archives before deriving a table parser or extraction tool.',
        rationale: 'The prefix words are useful structure evidence, but they are not decoded asset entries by themselves.',
      });
    }
  }

  if (/^(?:FULL3?|COMPACT3?|RESOURCE)\.CNF$/i.test(name)) {
    add(
      'pandemonium_pkg_asset_archives',
      'HIGH',
      `pandemonium.inventory.cnf:${normalizedPath}${sizeText}`,
      'Use CNF manifests to tie level/resource archives to repeatable extraction tests.',
      'The CNF files are the project-owned manifest surface that maps level and resource PKG archives.',
      ['CNF']
    );
  }

  if (/^MUSIC\/[0-9]+\.mp3$/i.test(normalizedPath)) {
    add(
      'pandemonium_music_assets',
      'MEDIUM',
      `pandemonium.inventory.music:${normalizedPath}${sizeText}`,
      'Keep music playback and replacement separate from graphics wrapper experiments.',
      'Music files are direct assets and reported audio issues can be confused with render-wrapper changes if they are tested together.',
      ['MP3']
    );
  }

  if (/^CHARLES\/(?:JEANFACE|KINGHEAD)\.BMP$/i.test(normalizedPath)) {
    add(
      'pandemonium_pkg_asset_archives',
      'MEDIUM',
      `pandemonium.inventory.charles_bmp:${normalizedPath}${sizeText}`,
      'Use Charles BMP examples as controlled import/export probes for JESTERS.PKG before broader texture replacement.',
      'The bundled Charles samples are known-good modding probes for a small part of the asset archive.',
      ['CHARLES', 'BMP']
    );
  }

  if (/^(?:win32|audiere)\.dll$/i.test(name)) {
    add(
      'pandemonium_settings_input_audio_risk',
      'MEDIUM',
      `pandemonium.inventory.runtime_support:${normalizedPath}${sizeText}`,
      'Classify runtime support DLLs separately from graphics wrappers when diagnosing settings, music, or menu crashes.',
      'Support DLLs may affect input, audio, or menu behavior and should not be treated as render-wrapper evidence.',
      ['runtime_support']
    );
  }
}

function scanPandemoniumCnfManifest(file, text, context) {
  if (!isPandemoniumProfile(context.profile) || !/(^|\/)(?:FULL3?|COMPACT3?|RESOURCE)\.CNF$/i.test(file.path)) {
    return;
  }
  ...
  for (let index = 0; index < lines.length; index += 1) {
    ...
    const match = rawLine.match(/^([A-Za-z0-9_.-]+\.PKG)\s+(-?\d+)\s+(-?\d+)\s+(-?\d+)\s+(-?\d+)\s+(-?\d+)\s+(-?\d+)\s*$/i);
    if (!match) {
      addEvidence(context, {
        category: 'pandemonium_cnf_manifest',
        severity: 'MEDIUM',
        confidence: 'possible',
        source_file: file.path,
        source_line_start: index + 1,
        source_line_end: index + 1,
        extracted_text: rawLine,
        normalized_claim: `pandemonium.cnf.unparsed:${file.path}:${rawLine}`,
        related_paths: [file.path],
        evidence_type: 'source_line',
        suggested_action: 'Review this CNF row before using the manifest as archive routing evidence.',
        rationale: 'Pandemonium CNF files are expected to map PKG archives through six numeric fields; unexpected rows need human review.',
      });
      continue;
    }

    const archive = match[1];
    const fields = match.slice(2).map(Number);
    addEvidence(context, {
      category: 'pandemonium_cnf_manifest',
      severity: 'HIGH',
      confidence: 'confirmed',
      ...
      source_line_end: index + 1,
      extracted_text: rawLine,
      normalized_claim: `pandemonium.cnf.archive:${file.path}:${archive}:fields=${fields.join(',')}`,
      related_paths: [file.path, archive],
      related_symbols: [archive],
      evidence_type: 'source_line',
      suggested_action: 'Use this CNF archive route to drive repeatable PKG extraction and level/resource mapping tests.',
      rationale: 'CNF rows provide structured archive routing evidence for Pandemonium levels and shared resources.',
    });
  }
}

function shouldProbePandemoniumPrefix(normalizedPath) {
  const name = basename(normalizedPath);
  return /^(?:PANDY3?|CHARLES)\.EXE$/i.test(name)
    || /^(?:glide|3DfxSpl|SST1INIT|win32|audiere|XANLIB|dgVoodoo|D3D8|D3D9|D3DImm|DDraw|dxgi|d3d11)\.dll$/i.test(name)
    || /\.(?:PKG|AVI)$/i.test(name)
    || /^MUSIC\/[0-9]+\.mp3$/i.test(normalizedPath)
    || /^CHARLES\/(?:JEANFACE|KINGHEAD)\.BMP$/i.test(normalizedPath);
}

async function readFilePrefixProbe(absPath, byteCount) {
  let handle;
  try {
    handle = await open(absPath, 'r');
    ...
    const u32Le = [];
    for (let offset = 0; offset + 4 <= prefix.length; offset += 4) {
      u32Le.push(prefix.readUInt32LE(offset));
    }
    return {
      hex: [...prefix.subarray(0, 32)]
...
function scanRuntimeKeywordFamilies(line, lineNo, file, context) {
  ...
  if (isDosRuntimeProfile(context.profile)) {
    return;
  }
...
function scanDosRuntimeKeywordFamilies(line, lineNo, file, context) {
  if (!isDosRuntimeProfile(context.profile) && !isWindowsPackageRuntimeProfile(context.profile)) {
    return;
  }

  const checks = [
    {
      pattern: /\b(DOSBox(?: Staging)?|dosbox-staging|dosbox\.conf|EmulatorRuntime|PCem|86Box|ScummVM)\b/i,
      category: 'dos_runtime_identity',
      severity: 'HIGH',
      action: 'Anchor the runtime to the exact DOS emulator/container, version, config file, and launch path before deriving fixes.',
      rationale: 'DOSBox, DOSBox Staging, PCem, 86Box, ScummVM, Wine, and native Windows have different timing, rendering, audio, and filesystem boundaries.',
    },
    {
      pattern: /\b(\[autoexec\]|mount\s+[a-z]\b|imgmount|boot\s+.*\.(?:img|ima)|-t\s+(?:cdrom|iso|floppy|hdd)|(?:\.cue|\.bin|\.iso|\.img)\b|drive_c|CDROM|MSCDEX)\b/i,
      category: 'dos_mount_image_paths',
      severity: 'HIGH',
      action: 'Verify drive letters, image paths, cue/bin pairing, executable path, and save/config location before changing launch scripts.',
      rationale: 'Old PC releases often fail because mounted drives and disc images do not match the expected install/runtime layout.',
    },
    {
      pattern: /\b(core\s*=|cycles\s*=|cputype\s*=|memsize\s*=|ems\s*=|xms\s*=|umb\s*=|machine\s*=|timed\s*=|fixed cycles|max cycles|dynamic core|normal core)\b/i,
      category: 'dos_cpu_memory_timing',
      severity: 'HIGH',
      action: 'Tune CPU core, cycles, memory, and timing as emulator runtime settings, then verify gameplay speed and FMV/audio sync.',
      rationale: 'DOS-era games are timing-sensitive; too-fast or mismatched emulation can look like renderer stutter or game logic failure.',
    },
    {
      pattern: /\b(output\s*=|scaler\s*=|aspect\s*=|fullresolution\s*=|windowresolution\s*=|glshader\s*=|texture_renderer|opengl|openglnb|surface|overlay|crt[-_ ]?shader|pixel[-_ ]?perfect)\b/i,
      category: 'dos_renderer_scaler_output',
      severity: 'MEDIUM',
      action: 'Treat output, scaler, aspect, resolution, and shader choices as emulator presentation settings, not DLL proxy wrapper evidence.',
      rationale: 'DOSBox rendering is inside the emulator container; it should not be merged with DXVK, dgVoodoo, Special K, or ReShade wrapper chains.',
    },
    {
      pattern: /\b(mididevice\s*=|mpu401\s*=|mt32|fluidsynth|soundfont|sbtype\s*=|oplmode\s*=|oplemu\s*=|gus\s*=|Gravis|Sound Blaster|General MIDI|Roland)\b/i,
      category: 'dos_audio_midi_stack',
      severity: 'MEDIUM',
      action: 'Capture MIDI, Sound Blaster, OPL, GUS, and soundfont settings separately from graphics fixes and validate sync in-game.',
      rationale: 'Audio and MIDI configuration are major stability and quality levers for DOS games, and they are independent of graphics wrappers.',
    },
    {
      pattern: /\b(joysticktype\s*=|mapperfile\s*=|keyboardlayout\s*=|autolock\s*=|sensitivity\s*=|usescancodes\s*=|controller|gamepad|mapper)\b/i,
      category: 'dos_input_mapper_timing',
      severity: 'MEDIUM',
      action: 'Keep mapper, joystick, keyboard layout, and autolock settings in an input lane with direct launch validation.',
      rationale: 'DOS input handling depends on emulator mapper and timing settings; mixing it with renderer changes hides real blockers.',
    },
    {
      pattern: /\b(container runtime|emulator container|not a DLL proxy|not a graphics wrapper|runtime containment|DOS runtime control lane|separate from wrapper chain)\b/i,
      category: 'dos_runtime_wrapper_boundary',
      severity: 'HIGH',
      action: 'Keep emulator/container evidence out of DLL proxy wrapper chains unless a separate host renderer wrapper is explicitly present.',
      rationale: 'The known wrapper-corpus weak spot was treating DOSBox internals as normal graphics-wrapper evidence.',
    },
  ];
...
function scanPandemoniumPcRuntimeLine(line, lineNo, file, context) {
  if (!isPandemoniumProfile(context.profile)) {
    return;
  }
  ...
  const checks = [
    {
      pattern: /\b(Pandemonium!?|PANDY3?\.EXE|SteamAppId\s*=?\s*243020|GOGID\s*=?\s*1207659054)\b/i,
      category: 'pandemonium_identity_surface',
      severity: 'HIGH',
      action: 'Anchor Pandemonium work to the exact executable, store edition, and hash before wrapper or patch experiments.',
      rationale: 'The Steam/GOG/retail identity controls executable names, bundled wrappers, registry paths, and fix compatibility.',
    },
    {
      pattern: /\b(Glide|glide\.dll|3DfxSpl|SST1INIT|nGlide|dgVoodoo|Direct3D ?11|Direct3D ?12|D3D11|D3D12|DXVK|Vulkan)\b/i,
      category: 'pandemonium_wrapper_chain',
      severity: 'HIGH',
      action: 'Classify wrapper order before testing: PANDY3.exe -> Glide -> nGlide or dgVoodoo -> Direct3D -> optional DXVK/Vulkan.',
      rationale: 'Pandemonium is a Glide-era game; stability depends on the exact translation chain and DLL bitness/load order.',
    },
    {
      pattern: /\b(PE32|Intel i386|32-?bit|x86\b|cannot load 64-?bit|64-?bit reimplementation|source port|UE5|Unreal Engine ?5)\b/i,
      category: 'pandemonium_32bit_boundary',
      severity: 'HIGH',
      action: 'Keep 32-bit wrapper stabilization separate from any 64-bit source-port or UE5 remaster plan.',
      rationale: 'A 32-bit Windows process cannot load 64-bit DLLs; a real 64-bit Pandemonium requires a new runtime that reads or converts original assets.',
    },
    {
      pattern: /\b(XanLib|xan_wc4|xan_dpcm|INTROS\.AVI|LOGOS\.AVI|OUTROS\.AVI|320x240|15 ?fps|FMV)\b/i,
      category: 'pandemonium_xan_video_assets',
      severity: 'MEDIUM',
      action: 'Treat FMV upgrades as a codec/timing replacement lane with original Xan WC4 decode proof and fallback files.',
      rationale: 'Pandemonium PC movies are old 320x240/15 FPS Xan streams; higher-quality replacements need playback compatibility or a new runtime.',
    },
    {
      pattern: /\b(\.PKG|JESTERS\.PKG|SJESTERS\.PKG|LEVEL[0-9]+3?\.PKG|RESOURCE\.CNF|FULL3?\.CNF|COMPACT3?\.CNF|CHARLES\.EXE|JEANFACE\.BMP|KINGHEAD\.BMP)\b/i,
      category: 'pandemonium_pkg_asset_archives',
      severity: 'HIGH',
      action: 'Map PKG headers, table entries, CNF level manifests, and Charles BMP tooling before attempting texture/model replacement.',
      rationale: 'The game is small, but assets are archive-packed; remaster work needs repeatable extraction/import evidence before patching.',
    },
    {
      pattern: /\b(PlayStation|PS1|PSX|MODE2\/2352|CD001|SYSTEM\.CNF|SLUS_[0-9.]+|PS-X EXE|\.cue|\.bin|disc image)\b/i,
      category: 'pandemonium_ps1_disc_reference',
      severity: 'HIGH',
      action: 'Treat the PlayStation disc as an owned local reference source; record hashes, cue/bin consistency, sector mode, and ISO file map before asset comparison.',
      rationale: 'The PS1 release is a separate runtime and asset source; it can guide remaster work, but raw copyrighted assets must stay local.',
    },
    {
      pattern: /\b(INTRO\.STR|LOGO\.STR|OUTRO\.STR|DUMMY\.STR|\.STR\b|MDEC|XA audio|XA-ADPCM|PlayStation FMV)\b/i,
      category: 'pandemonium_ps1_fmv_assets',
      severity: 'HIGH',
      action: 'Compare PS1 STR movie size, timing, dimensions, and decode quality against PC AVI files before choosing replacement assets.',
      rationale: 'The PS1 version uses STR FMVs that may be better source material than the PC Xan/AVI movies, but they need decode and timing proof.',
    },
    {
      pattern: /\b(PS1 JESTERS\.PKG|PlayStation JESTERS\.PKG|SLUS.*JESTERS|JESTERS\.PKG.*extent|JESTERS\.PKG.*u32le|asset reference)\b/i,
      category: 'pandemonium_ps1_pkg_reference',
      severity: 'HIGH',
      action: 'Compare PS1 and PC JESTERS.PKG sizes, header words, and entry tables before importing texture/model candidates.',
      rationale: 'A larger PS1 package is promising reference evidence, not proof that formats or assets are directly interchangeable.',
    },
    {
      pattern: /\b(HKCU\\SOFTWARE\\Crystal Dynamics\\Pandemonium|ESC|pause menu|settings|controller|joystick|music not playing|crash|WinMM|win32\.dll)\b/i,
      category: 'pandemonium_settings_input_audio_risk',
      severity: 'HIGH',
      action: 'Separate ESC/settings/controller/music crash fixes from graphics-wrapper experiments and capture registry before/after launch.',
      rationale: 'Public reports and PCGW fixes point at settings/menu/audio startup instability that can be masked by render wrapper changes.',
    },
    {
      pattern: /\b(PS1|PlayStation|texture upscale|model upgrade|remaster|asset extraction|UV|animation|proper PC port|source port)\b/i,
      category: 'pandemonium_remaster_lane',
      severity: 'MEDIUM',
      action: 'Track PS1-vs-PC asset quality, extraction state, and remaster target separately from original executable patches.',
      rationale: 'A proper PC remaster is an asset/runtime reconstruction project, not just a compatibility wrapper.',
    },
  ];
...
function scanWindowsPackageRuntimeKeywordFamilies(line, lineNo, file, context) {
  if (!isWindowsPackageRuntimeProfile(context.profile)) {
    return;
  }
  ...
  const checks = [
    {
      pattern: /\b(ArchiveLayout|PackageRoot|PortablePackage|RuntimeRoot|portable zip|portable package|archive layout|extracted folder|DOSBOX folder|SHADERS|launcher folder|clean test copy)\b/i,
      category: 'windows_package_layout',
      severity: 'HIGH',
      action: 'Map the package root, extracted layout, launch folder, config folder, shader folder, and test-copy boundary.',
      rationale: 'Old Windows game fixes often depend on relative package layout rather than system-wide install state.',
    },
    {
      pattern: /\b(LaunchExecutable|LaunchArguments|ConfigFile|ShaderPath|dosbox(?:-x)?\.conf|dosbox\.conf|fixvideo\.fx|pixelshader|glshader|fullscreen|fullresolution|windowresolution)\b/i,
      category: 'windows_package_runtime_config',
      severity: 'MEDIUM',
      action: 'Tie launch executable, config file, shader path, and presentation settings to the exact package layout.',
      rationale: 'Config and shader paths are package-relative and can silently break when a newer runtime has a different folder shape.',
    },
    {
      pattern: /\b(ExecutableBitness|ProcessBitness|HostBitness|PE32\+?|IMAGE_FILE_MACHINE|x86|x64|win32|win64|32-bit|64-bit|WOW64|dosbox_x64\.exe|dosbox-x\.exe)\b/i,
      category: 'windows_package_bitness',
      severity: 'HIGH',
      action: 'Verify target process bitness before selecting helper DLLs, wrapper DLLs, or replacement executables.',
      rationale: 'Windows DLL loading is architecture-specific; mixed 32/64-bit packages fail before wrapper behavior matters.',
    },
    {
      pattern: /\b(DLLLoadOrder|WrapperDLL|ProxyDLL|proxy DLL|load order|dinput8\.dll|dxgi\.dll|d3d11\.dll|d3d9\.dll|d3d8\.dll|ddraw\.dll|winmm\.dll|version\.dll|glide2x\.dll|3DfxSpl2\.dll|vulkan-1\.dll)\b/i,
      category: 'windows_package_dll_load_order',
      severity: 'HIGH',
      action: 'Inventory DLL role, process bitness, dependency chain, and rollback before stacking or replacing wrappers.',
      rationale: 'Wrapper packages can fight each other when proxy DLL order is guessed instead of recorded.',
    },
    {
      pattern: /\b(RuntimeDependencies|DLL dependencies|dependency|VC_redist|Visual C\+\+|DirectX Runtime|D3DCompiler|OpenAL|SDL2\.dll|MSVCP[0-9]+\.dll|VCRUNTIME[0-9]+\.dll)\b/i,
      category: 'windows_package_dependencies',
      severity: 'MEDIUM',
      action: 'Record dependency source, version, and target process before copying DLLs into the game folder.',
      rationale: 'Missing or mismatched redistributables can look like a wrapper bug even when the wrapper is fine.',
    },
    {
      pattern: /\b(LeastTranslationLayers|least translation|translation chain|translation layer|DXVK|D7VK|dgVoodoo|DDrawCompat|Special K|Lossless Scaling|LSFG|Vulkan wrapper|OpenGL output|Direct3D output)\b/i,
      category: 'windows_package_translation_chain',
      severity: 'HIGH',
      action: 'Choose the lowest-layer working runtime path first, then add wrappers one at a time with rollback evidence.',
      rationale: 'Every extra translation layer adds another failure surface for timing, input, frame pacing, and presentation.',
    },
    {
      pattern: /\b(RollbackPlan|backup|restore original|revert|uninstall|undo|clean copy|test copy|do not delete|preserve as control)\b/i,
      category: 'windows_package_rollback',
      severity: 'HIGH',
      action: 'Require a reversible test copy, hash notes, and restore path before changing local game/runtime package files.',
      rationale: 'Compatibility experiments need fast rollback because launch failures can come from any package layer.',
    },
  ];
...
function scanAgenticToolchainKeywordFamilies(line, lineNo, file, context) {
  ...
  const checks = [
    ...
    {
      pattern: /\b(voice note|voice message|voice input|transcription|transcribe|speech-to-text|text-to-speech|STT|TTS|Whisper|faster-whisper|Vosk|audio capture|audio queue|microphone|sample rate|language detect(?:ion)?)\b/i,
      category: 'voice_transcription_surface',
      severity: 'MEDIUM',
      action: 'Keep voice and transcription as an optional local-only lane with consent, redaction, retention, and no raw-audio commits.',
      rationale: 'Voice features can improve agent intake, but audio and transcripts are sensitive evidence that need explicit local storage and privacy boundaries.',
    },
    {
...
function isDosRuntimeProfile(profile) {
  return profile?.dosRuntimeProfile === true || [
    'dos_runtime',
    'emulator_runtime',
    'dosbox_runtime',
  ].includes(profile?.id);
...
function isPandemoniumProfile(profile) {
  return profile?.pandemoniumProfile === true || profile?.id === 'pandemonium_pc_runtime';
}
...
function isWindowsPackageRuntimeProfile(profile) {
  return profile?.windowsPackageRuntimeProfile === true || [
    'windows_package_runtime',
    'game_package_runtime',
  ].includes(profile?.id);
}
...
function isClaudeCodeModernProfile(profile) {
  return profile?.claudeCodeModernProfile === true
    || [CLAUDE_CODE_BASE_PROFILE_ID, ...CLAUDE_CODE_BASE_ALIASES].includes(profile?.id);
}
...
function summarize(evidence, contradictions, patchCandidates) {
  ...
  return {
    ...
    by_facet: summarizeEvidenceFacets(evidence),
    highest_severity: highestSeverity(evidence),
  };
}

function summarizeEvidenceFacets(evidence) {
  const facetKeys = [
    'subsystem',
    'authority',
    'source_boundary',
  ...
  for (const key of facetKeys) {
    summary[key] = countBy(evidence, item => item.facets?.[key] ?? 'unknown');
  }
  return summary;
}
...
function isReadOnlyCommand(command) {
  ...
  const withoutAllowedRedirects = trimmed.replace(/\s+\d?>\/dev\/null\b/g, '');
  if (hasUnsafeShellControl(withoutAllowedRedirects)) {
    return false;
  }
  const segments = splitShellPipeline(withoutAllowedRedirects);
...
function isReadOnlyCommandSegment(segment) {
  if (/\s-(?:exec|delete)\b/i.test(segment)) {
    return false;
  }
  const tokens = segment.trim().split(/\s+/);
  const executable = commandBasename(tokens[0]);
  if (executable === 'timeout') {
    const nestedExecutable = commandBasename(tokens[2]);
    return isKnownReadOnlyExecutable(nestedExecutable);
  }
  return isKnownReadOnlyExecutable(executable);
}
...
function isKnownReadOnlyExecutable(executable) {
  return /^(grep|find|test|sha256sum|node|sort|head|ps|ls|xdpyinfo|xset|glxinfo|vulkaninfo|env)$/i.test(executable);
}
...
function addEvidence(context, input) {
  ...
  evidence.facets = deriveEvidenceFacets({ ...evidence, facets: input.facets }, context);
...
function categorizeAssignment(key, value, profile) {
  ...
  if (isWindowsPackageRuntimeProfile(profile)) {
    if (/^(PACKAGEROOT|ARCHIVELAYOUT|PORTABLEPACKAGE|RUNTIMEROOT)$/i.test(key)
      || /\b(portable zip|portable package|archive layout|extracted folder|DOSBOX folder|SHADERS|clean test copy)\b/i.test(value)) {
      return 'windows_package_layout';
    }
    if (/^(LAUNCHEXECUTABLE|LAUNCHARGUMENTS|CONFIGFILE|SHADERPATH)$/i.test(key)
      || /\b(dosbox(?:-x)?\.conf|fixvideo\.fx|pixelshader|glshader|fullscreen|fullresolution|windowresolution)\b/i.test(value)) {
      return 'windows_package_runtime_config';
    }
    if (/^(EXECUTABLEBITNESS|PROCESSBITNESS|HOSTBITNESS|BITNESS)$/i.test(key)
      || /\b(PE32\+?|IMAGE_FILE_MACHINE|x86|x64|win32|win64|32-bit|64-bit|WOW64)\b/i.test(value)) {
      return 'windows_package_bitness';
    }
    if (/^(WRAPPERDLL|PROXYDLL|DLLLOADORDER)$/i.test(key)
      || /\b(dinput8\.dll|dxgi\.dll|d3d11\.dll|d3d9\.dll|d3d8\.dll|ddraw\.dll|winmm\.dll|version\.dll|glide2x\.dll|3DfxSpl2\.dll|vulkan-1\.dll)\b/i.test(value)) {
      return 'windows_package_dll_load_order';
    }
    if (/^(RUNTIMEDEPENDENCIES|DLLDEPENDENCIES|VC_REDIST|DIRECTXRUNTIME)$/i.test(key)
      || /\b(Visual C\+\+|DirectX Runtime|D3DCompiler|OpenAL|SDL2\.dll|MSVCP[0-9]+\.dll|VCRUNTIME[0-9]+\.dll)\b/i.test(value)) {
      return 'windows_package_dependencies';
    }
    if (/^(LEASTTRANSLATIONLAYERS|TRANSLATIONCHAIN)$/i.test(key)
      || /\b(least translation|translation chain|DXVK|D7VK|dgVoodoo|DDrawCompat|Special K|Lossless Scaling|LSFG)\b/i.test(value)) {
      return 'windows_package_translation_chain';
    }
    if (/^(ROLLBACKPLAN|BACKUPPATH)$/i.test(key)
      || /\b(restore original|revert|uninstall|clean copy|test copy|preserve as control)\b/i.test(value)) {
      return 'windows_package_rollback';
    }
  }
  ...
  if (isDosRuntimeProfile(profile)) {
    if (/^(DOSBOX|DOSBOXVERSION|DOSBOXSTAGING|EMULATORRUNTIME|EMULATOR|RUNTIME)$/i.test(key)
      || /\b(DOSBox(?: Staging)?|dosbox-staging|dosbox\.conf|PCem|86Box|ScummVM)\b/i.test(value)) {
      return 'dos_runtime_identity';
    }
    if (/^(AUTOEXEC|MOUNT|IMGMOUNT|IMG_MOUNT|CDROM|DRIVEC|DRIVE_C|CUE|BIN|ISO|IMAGEPATH|DISCIMAGE)$/i.test(key)
      || /\b(mount\s+[a-z]\b|imgmount|boot\s+.*\.(?:img|ima)|-t\s+(?:cdrom|iso|floppy|hdd)|(?:\.cue|\.bin|\.iso|\.img)\b|drive_c|CDROM|MSCDEX)\b/i.test(value)) {
      return 'dos_mount_image_paths';
    }
    if (/^(CORE|CYCLES|CPUTYPE|CPU_TYPE|MEMSIZE|MEM_SIZE|EMS|XMS|UMB|MACHINE|TIMED)$/i.test(key)
      || /\b(core\s*=|cycles\s*=|cputype\s*=|memsize\s*=|ems\s*=|xms\s*=|umb\s*=|machine\s*=|timed\s*=|fixed cycles|max cycles|dynamic core|normal core)\b/i.test(value)) {
      return 'dos_cpu_memory_timing';
    }
    if (/^(OUTPUT|SCALER|ASPECT|FULLSCREEN|FULLRESOLUTION|WINDOWRESOLUTION|FULL_RESOLUTION|WINDOW_RESOLUTION|GLSHADER|GL_SHADER|TEXTURE_RENDERER)$/i.test(key)
      || /\b(output\s*=|scaler\s*=|aspect\s*=|fullresolution\s*=|windowresolution\s*=|glshader\s*=|texture_renderer|opengl|openglnb|surface|overlay|crt[-_ ]?shader|pixel[-_ ]?perfect)\b/i.test(value)) {
      return 'dos_renderer_scaler_output';
    }
    if (/^(MIDIDEVICE|MIDI_DEVICE|MPU401|MT32|FLUIDSYNTH|SOUNDFONT|SBTYPE|SB_TYPE|OPLMODE|OPL_MODE|OPLEMU|GUS)$/i.test(key)
      || /\b(mididevice\s*=|mpu401\s*=|mt32|fluidsynth|soundfont|sbtype\s*=|oplmode\s*=|oplemu\s*=|gus\s*=|Gravis|Sound Blaster|General MIDI|Roland)\b/i.test(value)) {
      return 'dos_audio_midi_stack';
    }
    if (/^(JOYSTICKTYPE|JOYSTICK_TYPE|MAPPERFILE|MAPPER_FILE|KEYBOARDLAYOUT|KEYBOARD_LAYOUT|AUTOLOCK|SENSITIVITY|USESCANCODES)$/i.test(key)
      || /\b(joysticktype\s*=|mapperfile\s*=|keyboardlayout\s*=|autolock\s*=|sensitivity\s*=|usescancodes\s*=|controller|gamepad|mapper)\b/i.test(value)) {
      return 'dos_input_mapper_timing';
    }
    if (/^(CONTAINERRUNTIME|CONTAINER_RUNTIME|WRAPPERBOUNDARY|WRAPPER_BOUNDARY|RUNTIMECONTAINMENT)$/i.test(key)
      || /\b(container runtime|emulator container|not a DLL proxy|not a graphics wrapper|runtime containment|DOS runtime control lane|separate from wrapper chain)\b/i.test(value)) {
      return 'dos_runtime_wrapper_boundary';
    }
  }
...
function severityForAssignment(key, category) {
  ...
  if (category === 'pcgw_availability_drm'
    ...
    || category === 'windows_pe_metadata'
    || category === 'windows_installer_layout'
    || category === 'windows_package_binary'
    || category === 'windows_package_runtime_config'
    || category === 'windows_package_dependencies'
    || category === 'windows_package_shader_asset') {
    return 'MEDIUM';
  }
...
function isDefinitionCategory(category) {
  return [
    ...
    'dos_mount_image_paths',
    'dos_cpu_memory_timing',
    'dos_renderer_scaler_output',
    'dos_audio_midi_stack',
    'dos_input_mapper_timing',
    'dos_runtime_wrapper_boundary',
    ...
    'windows_package_dll_load_order',
    'windows_package_runtime_config',
    'windows_package_dependencies',
    'windows_package_translation_chain',
    'windows_package_rollback',
    'windows_package_shader_asset',
  ].includes(category);
}
