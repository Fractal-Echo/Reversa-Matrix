...
const SEMANTIC_POLICY_CONFLICT_RULES = [
  {
    ...
    left: ['read_only', 'source_patch_forbidden'],
...
async function scanFile(file, context) {
  ...
  const lineState = { configSection: null, markdownFence: false };
...
function scanSemanticPolicyClaims(line, lineNo, file, context, lineState = {}) {
  ...
  const trimmed = line.trim();
  const evidenceKind = semanticEvidenceKind(file.path);
...
function semanticEvidenceKind(filePath) {
...
function isGeneratedArtifactPath(filePath) {
  ...
  return ROOT_SCAN_ARTIFACT_FILES.has(normalized)
    ...
    || /(^|\/)(report|evidence|summary|dashboard|compare_report|compare_summary)\.(json|jsonl|md|html)$/i.test(normalized)
...
function isDefinitionGroupingEligible(item) {
  return item.assignment_operator !== '+='
    && !isAuxiliarySourcePath(item.source_file);
}
