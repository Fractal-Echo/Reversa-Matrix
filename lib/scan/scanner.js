...
async function walkProject(dir, context, options, relDir = '') {
  ...
  for (const entry of entries) {
    ...
    if (entry.isDirectory()) {
      const skipReason = directorySkipReason(entry.name, relPath, context.projectRoot, options.outDir, context.ignoredPaths);
      if (skipReason) {
...
function directorySkipReason(name, relPath, projectRoot, outDir, ignoredPaths) {
...
async function scanFile(file, context) {
  ...
  const lineState = { configSection: null };
...
function updateLineState(line, file, state) {
  ...
  const section = line.trim().match(/^\[([A-Za-z0-9_.-]+)]\s*$/);
  if (section) {
...
function scanLine(line, lineNo, file, context, lineState = {}) {
  ...
  if (placeholder && !isPlaceholderExample(trimmed, placeholder[1])) {
...
function isPlaceholderExample(line, marker) {
  if (marker !== 'XXX') {
    return false;
  }

  return /(?:^|[^A-Za-z0-9])(?:[A-Z][A-Z0-9-]*-)?XXX(?:[^A-Za-z0-9]|$)/.test(line)
    && /<[^>]*XXX[^>]*>|[A-Z][A-Z0-9-]*-XXX/.test(line);
}
...
function scanAssignment(line, lineNo, file, context, lineState = {}) {
  ...
  let category = categorizeAssignment(key, rawValue);
  const localCodeAssignment = isLocalCodeAssignment(key, file.path);
  if (localCodeAssignment || (category === 'build_variables' && !isProjectLevelAssignment(key, file.path))) {
...
function isLocalCodeAssignment(key, filePath) {
  ...
  if (isProjectLevelAssignment(key, filePath)) {
    return false;
  }

  const name = filePath.split('/').pop() ?? filePath;
  if (/\.(py|js|jsx|ts|tsx|mjs|cjs|sh|bash|zsh|fish|rb|go|rs|java|kt|kts|c|cc|cpp|h|hpp|cs|php|swift|lua|pl|ps1)$/i.test(name)) {
    return true;
  }

  return /(^|\/)(templates?|examples?|fixtures?|scripts?)\//i.test(filePath);
}
...
function isDefinitionGroupingEligible(item) {
  return !isAuxiliarySourcePath(item.source_file);
}
...
function isLikelyProseCompoundPath(pathRef) {
  return /^(kernel|device|proprietary)\/[A-Za-z0-9.-]+$/.test(pathRef)
