...
function scanStructuredJsonConfig(file, text, context) {
  ...
  for (const entry of collectStructuredJsonAssignments(parsed)) {
    ...
    const evidence = addEvidence(context, {
      ...
      normalized_claim: `${entry.key}=${normalizedValue}`,
      assignment_operator: '=',
...
function packageConfigAssignmentScope(filePath) {
  ...
  if (name !== '.editorconfig' && name !== 'pyproject.toml') {
    return null;
  }
...
function scanSemanticPolicyClaims(line, lineNo, file, context, lineState = {}) {
  ...
  for (const rule of SEMANTIC_POLICY_CLAIM_RULES) {
    ...
    if (isSemanticPolicyRuleFalsePositive(trimmed, file.path, rule.category)) {
      continue;
    }
...
function isSemanticPolicyRuleFalsePositive(trimmed, filePath, category) {
...
function shouldEnforceRequiredFilePatterns(context, requiredPatterns) {
  ...
  const hasRequiredFileAnchor = context.inventory.files.some(file => requiredPatterns.some(pattern => pattern.test(file.path)));
  if (hasRequiredFileAnchor) return true;

  const hasRecoveryTreeAnchor = context.inventory.files.some(file => isAndroidRecoveryTreeAnchor(file.path));
  if (hasRecoveryTreeAnchor) return true;

  return context.evidence.some(item => isAndroidRecoveryIdentityClaim(item.normalized_claim));
}

function isAndroidRecoveryTreeAnchor(filePath) {
  return /(^|\/)(BoardConfig.*\.mk|AndroidProducts\.mk|device[^/]*\.mk|product[^/]*\.mk|orangefox_.*\.mk|twrp_.*\.mk|vendorsetup\.sh|vendor-files.*\.txt|proprietary-files.*\.txt|extract-files.*\.sh|.*fstab.*)$/i.test(filePath)
    || /(^|\/)(recovery\/root|root\/(?:etc|sbin|system))(\/|$)/i.test(filePath);
}

function isAndroidRecoveryIdentityClaim(claim) {
  return /^(TARGET_BOARD_PLATFORM|PRODUCT_PLATFORM|PRODUCT_DEVICE|PRODUCT_MODEL|PRODUCT_NAME|TARGET_DEVICE|TARGET_OTA_ASSERT_DEVICE|TARGET_BOOTLOADER_BOARD_NAME|TARGET_RECOVERY_|BOARD_|TW_|OF_|FOX_)/.test(String(claim ?? ''));
}
...
function shouldCheckPath(pathRef, category = 'path_reference', profile = null) {
  ...
  if (isPowerTdpRuntimeProfile(profile) && isPowerRuntimeTargetPath(pathRef)) {
    return false;
  }
  if (/[*$?{}]|^\$\(|\$\(/.test(pathRef)) {
    return false;
