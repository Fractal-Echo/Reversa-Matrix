...
export function buildSummaryMarkdown(report) {
  ...
  if (report.summary.by_facet) {
    lines.push('## 24-TET Facet Map');
    lines.push('');
    for (const [facet, counts] of Object.entries(report.summary.by_facet)) {
      lines.push(`### ${facet}`);
      lines.push('');
      lines.push(markdownTable(
        ['Value', 'Count'],
  ...
  const facetCards = topFacetEvidenceCards(report);
  if (facetCards.length > 0) {
    lines.push('## Top Evidence Facet Cards');
    lines.push('');
    for (const item of facetCards) {
      lines.push(`### ${item.category} - ${item.normalized_claim}`);
      lines.push('');
      lines.push(`- Source: \`${item.source_file}:${item.source_line_start}\``);
      lines.push(`- Severity/confidence: \`${item.severity}/${item.confidence}\``);
      lines.push(`- Facets: ${facetInlineMarkdown(item.facets)}`);
      lines.push(`- Next artifact: \`${item.facets.required_next_artifact}\``);
      lines.push('');
    }
  }
...
<body>
  ...
  <main>
    ...
    <section>
      <h2>24-TET Facet Cards</h2>
      ${facetCards.length === 0 ? '<p class="muted">No facet cards available.</p>' : htmlFacetCards(facetCards)}
    </section>
...
function evidenceTable(items) {
  return htmlTable(
    ['Severity', 'Confidence', 'Category', 'Claim', 'Facets', 'Source'],
    items.map(item => [
      severityBadge(item.severity),
      item.confidence,
      item.category,
      item.normalized_claim,
      item.facets ? rawHtml(facetChips(item.facets)) : '(none)',
      `${item.source_file}:${item.source_line_start}`,
    ])
  );
}

function topFacetEvidenceCards(report) {
  return (report.evidence ?? [])
    .filter(item => item.facets)
    .sort((a, b) => facetCardRank(b) - facetCardRank(a) || compareFindings(a, b))
    .slice(0, 12);
}

function facetCardRank(item) {
  const facets = item.facets ?? {};
  let score = severityRank(item.severity) * 10;
  if (facets.authority === 'raw_artifact') score += 20;
  if (facets.authority === 'source_line') score += 8;
  if (facets.source_boundary === 'source_authority') score += 14;
  if (facets.source_boundary === 'generated_non_authority') score -= 20;
  if (facets.proof_level === 'artifact_backed') score += 18;
  if (facets.operator_steer_state === 'confirmed_or_locked') score += 14;
  if (facets.mutation_risk === 'boot_or_kernel_rebuild_required') score += 8;
  if (facets.deterministic_truth_above_model) score += 2;
  return score;
}

function htmlFacetCards(items) {
  return `<div class="facet-card-grid">${items.map(item => `
    <article class="facet-card">
      <h3>${escapeHtml(item.category)} - ${escapeHtml(item.normalized_claim)}</h3>
      <p class="muted">${escapeHtml(item.source_file)}:${escapeHtml(item.source_line_start)}</p>
      <p>${facetChips(item.facets)}</p>
      <p class="facet-next">Next artifact: ${escapeHtml(item.facets.required_next_artifact)}</p>
    </article>
  `).join('\n')}</div>`;
}
...
function facetChips(facets) {
  return [
    ...
    ['boundary', facets.source_boundary],
    ['risk', facets.mutation_risk],
    ['proof', facets.proof_level],
    ['steer', facets.operator_steer_state],
    ['model', facets.model_role],
  ]
    .filter(([, value]) => value)
    .map(([key, value]) => `<span class="facet-chip">${escapeHtml(key)}=${escapeHtml(value)}</span>`)
    .join(' ');
}

function facetInlineMarkdown(facets) {
  return [
    ['subsystem', facets.subsystem],
    ['authority', facets.authority],
    ['boundary', facets.source_boundary],
    ...
    ['steer', facets.operator_steer_state],
    ['model', facets.model_role],
    ['truth_above_model', String(facets.deterministic_truth_above_model)],
  ]
    .filter(([, value]) => value)
